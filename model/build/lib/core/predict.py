...
class Predict:
    def __init__(self):
        if os.path.isfile('model/data/clean/dataLimits.xlsx'):
            self.indf = pd.read_excel('model/data/clean/scaledin.xlsx').drop('Unnamed: 0', axis=1)
            self.outdf = pd.read_excel('model/data/clean/scaledout.xlsx').drop('Unnamed: 0', axis=1)
        ...
        for file in filenames[:-8]:
            ...
            self.models[file].load_state_dict(torch.load('model/core/models/' + file))
    ...
    def prepare_data(self, df: pd.DataFrame):
        ...
        # change to float
        df[self.indf.columns.tolist()] = df[self.indf.columns.tolist()].replace('', 0).astype(np.float32)
